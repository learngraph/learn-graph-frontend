...
export const DeletePlaygroundGraphButton = (props : {ctrl: Controller}) => {
  ...
  const handleDelete = () => {
    ...
    props.ctrl.graph.current.links.forEach(link => props.ctrl.graph.removeLink(link));
    props.ctrl.graph.current.nodes.forEach(node => props.ctrl.graph.removeNode(node));
